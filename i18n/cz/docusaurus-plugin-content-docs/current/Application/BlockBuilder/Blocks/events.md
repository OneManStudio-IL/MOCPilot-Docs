...
### `Když program začne` (Začátečník) {#block_event_when_program_started}
...
### `Když` (Pokročilý) {#block_event_when}
...
### `Když časovač je větší než` (Pokročilý) {#block_event_when_timer_greater_than}
...
### `Když je přijata zpráva` (Guru) {#block_event_when_message_recieved}
...
### `Vyslat` (Guru) {#block_event_broadcast}
...
### `Vyslat a čekat` (Guru) {#block_event_broadcast_and_wait}
...
### `Stisk tlačítka hubu` (Začátečník) {#block_hubs_all_event_button_pressed}
...
### `Změna připojení hubu` (Začátečník) {#block_hubs_all_event_connection_changed}
...
### `Změna baterie hubu` (Začátečník) {#block_hubs_all_event_battery_changed}
...
### `Změna akcelerometru hubu` (Guru) {#block_hubs_all_event_accelerometer_changed}
...
### `Změna náklonu hubu` (Začátečník) {#block_hubs_all_event_tilt_changed}
...
### `Změna orientace hubu` (Začátečník) {#block_hubs_all_event_tilt_orientation_changed}
...
### `Událost barvy senzoru Technic` (Začátečník) {#block_hubs_all_event_port_technic_sensor_color}
...
### `Událost vzdálenosti senzoru Technic` (Začátečník) {#block_hubs_all_event_port_technic_sensor_distance}
...
### `Událost vzdálenosti senzoru BOOST` (Začátečník) {#block_hubs_all_event_port_boost_sensor_when_distance}
...
### `Událost barvy senzoru BOOST` (Začátečník) {#block_hubs_all_event_port_boost_sensor_color}
