...
### `Erstatt på` (Guru) {#block_lists_replace_at}
...
### `Slett` (Avansert) {#block_lists_delete}
...
### `Tøm alle` (Avansert) {#block_lists_clear_all}
...
### `Inneholder verdi` (Guru) {#block_lists_contains_value}
...
### `Størrelse på liste` (Avansert) {#block_lists_size_of_value}
