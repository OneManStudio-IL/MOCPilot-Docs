...
### `等待` (初级) {#block_control_wait_for}

<img src={useBaseUrl('/img/blocks/block_control_wait_for.svg')} alt="block_control_wait_for.svg" />

将当前脚本暂停指定时间。

### `等待直到` (高级) {#block_control_wait_until}
...
### `如果` (初级) {#block_control_if}

<img src={useBaseUrl('/img/blocks/block_control_if.svg')} alt="block_control_if.svg" />

仅在条件为真时执行嵌套积木。

### `如果 / 否则` (高级) {#block_control_if_else}

<img src={useBaseUrl('/img/blocks/block_control_if_else.svg')} alt="block_control_if_else.svg" />

当条件为真时执行一个分支，否则执行另一个分支。

### `执行这个和这个` (专家) {#block_control_do_this_and_this}
...
### `停止其他积木堆` (专家) {#block_control_stop_other_stacks}
...
### `设置连接` (初级) {#block_hubs_control_set_connect}
...
### `设置 Hub 配置` (高级) {#block_hubs_set_hub_config}
<img src={useBaseUrl('/img/blocks/block_hubs_set_hub_config.svg')} alt="block_hubs_set_hub_config.svg" />
将所选 Hub 切换到其支持的配置文件之一。

- 仅适用于提供 Hub 配置选项的 Hub。
- 典型用途：在运行后续积木前，将可配置的 Hub（例如 Technic Move Hub）切换到另一个已保存配置。

### `Hub 配置` (高级) {#block_hubs_all_control_hub_config}
...
- 输出格式选项：`text`、`index`
- 仅适用于提供 Hub 配置选项的 Hub。
- 当所选 Hub 未连接或没有可用配置时返回 `NaN`。

### `是否已连接` (初级) {#block_hubs_all_sensors_is_connected}
...
### `BuWizz 2 设置功率模式` (初级) {#block_hubs_buwizz_sensors_set_power_mode}
...
### `BuWizz 2 获取功率模式` (专家) {#block_hubs_buwizz_sensors_get_power_mode}
...
### `MouldKing 设置控制通道` (高级) {#block_hubs_mouldking_control_set_control_channel}
...
### `MouldKing 获取控制通道` (高级) {#block_hubs_mouldking_control_get_control_channel}
